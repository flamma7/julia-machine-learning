...
# ╔═╡ c38a3ecc-61e4-11f0-110d-dbabb3ae200d
using MLJ, Plots, Statistics, LinearAlgebra, DataFrames, PlutoUI

# ╔═╡ 1415badf-6cb9-4d39-bb7b-c3d17ed30219
using JuMP, Ipopt

# ╔═╡ 1b2bd6ec-cec7-46c6-b2a7-864ddfea20a0
import CSV

# ╔═╡ 80f09960-fd88-4f6c-9a58-19f395813698
@bind filtered_data CheckBox(default=false)

# ╔═╡ 6b8620e3-bc2f-459c-bf55-5c7fb12245c1
begin
	...
	df_val = CSV.read("val.csv", DataFrame)
	df_test = CSV.read("test.csv", DataFrame)
end

# ╔═╡ a4ab1bd8-7f1e-466a-b96f-90f906907267
features = names(df_train, Not([:quality, :qualityC]))

# ╔═╡ 5d922802-7985-4e13-947a-6a56187d6688
# Make just 2 class
begin
	df_train_2 = filter(x -> x.qualityC != "medium", df_train)
	df_val_2 = filter(x -> x.qualityC != "medium", df_val)
	df_test_2 = filter(x -> x.qualityC != "medium", df_test)
end

# ╔═╡ 2983bf3a-ec32-4b81-aa2a-ae404202f25c
begin
	X_train = Matrix(df_train_2[:, features])
	X_val = Matrix(df_val_2[:, features])
	X_test = Matrix(df_test_2[:, features])

	t_train = ifelse.(df_train_2.qualityC .== "low", -1, 1)
	t_val = ifelse.(df_val_2.qualityC .== "low", -1, 1)
	t_test = ifelse.(df_test_2.qualityC .== "low", -1, 1)
end

# ╔═╡ b74707be-695c-4fae-a609-c4f25b460388
N, M = size(X_train)

# ╔═╡ 16b87b3a-dba7-4ae5-b2d7-55106d618aa2
model = JuMP.Model(Ipopt.Optimizer)

# ╔═╡ f0c12a58-a0d2-4c74-869f-03391b2498d6
set_optimizer_attribute(model, "print_level", 0)

# ╔═╡ 70aa8fcc-bdd9-45ca-84bc-4b9a94ce99e4
Q = zeros(N, N)

# ╔═╡ c4ca4e1f-2afb-4233-ae57-ac563f238da9
function compute_kernel(x1, x2)
	dot(x1, x2)
end

# ╔═╡ 525044d5-d5c6-4aad-bb66-f9de2a074ec2
for i in 1:N
	for j in 1:N
		Q[i,j] = t_train[i] * t_train[j] * compute_kernel(X_train[i,:], X_train[j,:])
	end
end

# ╔═╡ 7c16fb31-4ca6-4125-a3bc-318f33e63434
Q

# ╔═╡ d7935ee1-0752-4ec1-98cf-462a680d922f
#p = ones(N) * -1
p = ones(N)

# ╔═╡ 1d91f93b-a413-445c-8f81-b5c694a158bf
begin
	@variable(model, α[1:N] >= 0)
	@objective(model, Max, -0.5 * dot(α, Q * α) + dot(p, α))
end

# ╔═╡ 2a06005b-f190-4274-b500-815fa0bfe9bf
@constraint(model, sum(α[i] * t_train[i] for i in 1:N) == 0)

# ╔═╡ f4d16744-0d52-44d5-a70f-22583320e909
C = 10.0

# ╔═╡ da5d9ce7-5f59-41aa-a342-e79bbd12acfb
for i in 1:N
	@constraint(model, α[i] <= C)
end

# ╔═╡ c79eff9a-2563-4f1d-a300-d4eaa3a07c43
optimize!(model)

# ╔═╡ f70a1d21-1e81-44a8-bdb0-8a5e75921a13
model

# ╔═╡ 197c8f28-0215-4a20-bb36-5dca145445c0
α_opt = value.(α)

# ╔═╡ 637dd53f-5407-432d-a058-0f8660238d1b
ϵ = 1e-5

# ╔═╡ a546e2b5-aaba-4809-8e35-246ead1f010c
num_support_vecs = count(x -> abs(x) > ϵ, α_opt)

# ╔═╡ 5f11540d-69c0-4dfb-bad3-b68e4bb7ff23
is_sv_index = ifelse.(α_opt .> ϵ, 1, 0)

# ╔═╡ 09a77afc-f50b-4f6c-8acd-59ae593d8172
N

# ╔═╡ d0703fab-6e76-4935-81e0-abdc1626ac75
begin
	K = zeros(N,N)
	for i in 1:N
		for j in 1:N
...
# ╔═╡ 7c9b520a-51ec-45d1-86ba-4fc607ed11f3
prod = (t_train .* is_sv_index) .- sum(K .* α_opt .* t_train .* is_sv_index, dims=2)

# ╔═╡ f52c7f43-9c46-4bc2-854a-2a4d091e49e5
b = sum(prod) / num_support_vecs

# ╔═╡ 3a7d67ef-4d6d-4684-aa36-c4cf3a251e80
# Get training set error
y_train = K * (α_opt .* t_train) .+ b

# ╔═╡ 1e5a8caf-2fd7-40e8-8a15-bd11c30a3368
y_train_sign = sign.(y_train)

# ╔═╡ 557ae41a-7b47-4b57-9cd4-fde6db0b9846
accuracy(y_train_sign, t_train)

# ╔═╡ a3e55eac-df59-4e67-aaba-120a4d838cb8
N_test = length(t_test)

# ╔═╡ 9f370820-5d3a-47f2-a737-8e000d216af9
begin
	K_test = zeros(N_test, N)
	for i in 1:N_test
		for j in 1:N
			K_test[i,j] = compute_kernel(X_test[i,:], X_train[j,:])
		end
	end
end

# ╔═╡ 192831db-38da-4de8-8960-c7b7795c0cba
y_test = sign.( K_test * (α_opt .* t_train))

# ╔═╡ efc16a5c-7553-4848-9b16-5f158dd6fe1d
accuracy(y_test, t_test)

# ╔═╡ 0934b814-c571-4c42-9b8d-5f8d1f94abda
sum([1 2; 3 4], dims=2)
...
manifest_format = "2.0"
project_hash = "bb036244392888a26da3b6e41f931ca2d8d59e6e"
