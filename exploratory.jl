...
# ╔═╡ 42c1f37c-59a9-11f0-0d8e-9163aa7c161a
using Plots, CSV, DataFrames

# ╔═╡ ef4ab84e-48ac-4e6f-9a45-5d4a4ac7d160
df = CSV.read("train.csv", DataFrame)

# ╔═╡ 9979bc88-70bf-4659-91e4-123d0c988b2a
features = names(df, Not([:quality, :qualityC]))

# ╔═╡ 57c48775-9935-426b-9550-15dee496670e
visualizations = [df[:,col] for col in features]

# ╔═╡ 112264d6-1651-4048-bf63-0592674b1ea4
histogram(visualizations, layout=(4,3), title=reshape(features, 1, length(features)), size=(800,800), legend=false)

# ╔═╡ a5b1970d-c8f0-4835-b92e-a22035885916
for col in features
	max = round(maximum(df[:, col]), digits=1)
	min = round(minimum(df[:, col]), digits=1)
	println("$col [$min, $max]")
end

# ╔═╡ fcfdfe03-3ca9-4cab-9c13-9f5ec144021c
scatter(df.qualityC, visualizations, layout=(4,3), title=reshape(features, 1, length(features)), size=(800,800), legend=false, markersize=3, markerstrokewidth=0)
