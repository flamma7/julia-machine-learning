...
# ╔═╡ e4603c0c-3760-49ee-b3bf-07dd56d51764
using Plots, Statistics, MLJ, DataFrames, LinearAlgebra

# ╔═╡ 6a8f32cb-220e-4315-8402-ab94917d0904
using PlutoUI, LaTeXStrings

# ╔═╡ f2398c04-5bff-11f0-1aa6-29dfc9df57d8
import CSV

# ╔═╡ d30f13d5-d537-4ef0-bb81-9476b5355f33
@bind filtered_data CheckBox(default=false)

# ╔═╡ 9e88ec12-ef8d-4d8f-8e9b-9bd480c3334f
@bind use_mahalanobis CheckBox(default=false)

# ╔═╡ e551f6cf-b3f8-4c04-84f1-43c390121e34
begin
	df_train_all = CSV.read("train.csv", DataFrame)
	df_train_filtered = CSV.read("train_f.csv", DataFrame)
	df_train = filtered_data ? df_train_filtered : df_train_all
	df_test = CSV.read("test.csv", DataFrame)
end

# ╔═╡ 2c2b252e-b991-49a4-b4d4-ee840ee32e2d
features = names(df_train, Not([:quality, :qualityC]))

# ╔═╡ 4620116d-6cd6-41ff-9fe8-815b466592cb
begin
	low_train = filter(x -> x.qualityC == "low", df_train)
	med_train = filter(x -> x.qualityC == "medium", df_train)
	high_train = filter(x -> x.qualityC == "high", df_train)
	X_low = Matrix(low_train[:, features])
	X_med = Matrix(med_train[:, features])
	X_high = Matrix(high_train[:, features])
	X_train = Matrix(df_train[:, features])
	X_test = Matrix(df_test[:, features])
	μ_low = mean(X_low, dims=1)
	μ_med = mean(X_med, dims=1)
	μ_high = mean(X_high, dims=1)
	μ_train = mean(X_train, dims=1)
end

# ╔═╡ b130f7a4-bcf7-4a52-94f3-c7cb15ba1660
begin
	low_cent = X_low .- μ_low
	med_cent = X_med .- μ_med
	high_cent = X_high .- μ_high

	Σ_low = low_cent' * low_cent
	Σ_med = med_cent' * med_cent
	Σ_high = high_cent' * high_cent

	Sw = Σ_low + Σ_med + Σ_high
end

# ╔═╡ 7118dee7-b4f8-4b07-8665-b41bf01d9e7e
begin
	diff_low = μ_low - μ_train
	diff_med = μ_med - μ_train
	diff_high = μ_high - μ_train

	Sb = nrow(low_train) * diff_low'*diff_low + nrow(med_train)*diff_med'*diff_med + nrow(high_train) * diff_high'*diff_high
end

# ╔═╡ 4557e541-72e0-487c-8214-55354f28d345
eig = eigen(inv(Sw) * Sb)

# ╔═╡ 1be293cc-1957-41f5-8df7-da8817ea72ea
# Get indices of two largest eigenvalues by magnitude
idx = sortperm(abs.(eig.values), rev=true)[1:2]

# ╔═╡ a1018a38-97e3-481e-b75a-2ae2df5e813b
# Create matrix W with corresponding eigenvectors
W = real(eig.vectors[:, idx])

# ╔═╡ 7f428736-d0a2-474b-9433-15a1687b310f
begin
	z_low = W' * μ_low'
	z_med = W' * μ_med'
	z_high = W' * μ_high'
...
# ╔═╡ c79c7de8-6259-4616-a318-c23be54e72bb
Σ = W' * Sw * W

# ╔═╡ de1aa9fb-49fa-49a9-b17f-bc3b6d969cf4
function mahalanobis_distance(z, μ)
	sqrt((z-μ)' * inv(Σ) * (z-μ))[1,1]
end

# ╔═╡ c34dcd7f-a896-47a5-b863-30291697b070
function euclidean_distance(z,μ)
	norm(z-μ)
end

# ╔═╡ be1707f3-d700-4922-9e1d-47080b1d4184
z_train = X_train * W

# ╔═╡ 89cad116-a6ad-4010-89f6-f4566ee141fe
function find_class_euclid(z)
	d_low = euclidean_distance(z, z_low)
	d_med = euclidean_distance(z, z_med)
	d_high = euclidean_distance(z, z_high)
	if d_low < d_med && d_low < d_high
		return "low"
	elseif d_med < d_low && d_med < d_high
		return "medium"
	else
		return "high"
	end
...
# ╔═╡ ad355854-046d-43ce-b5e4-58ee39de4866
function find_class_mahala(z)
	d_low = mahalanobis_distance(z, z_low)
	d_med = mahalanobis_distance(z, z_med)
	d_high = mahalanobis_distance(z, z_high)
	if d_low < d_med && d_low < d_high
		return "low"
	elseif d_med < d_low && d_med < d_high
		return "medium"
...
# ╔═╡ 9fc1c9af-6ded-4d13-bfbe-04c96732a36a
y_train = use_mahalanobis ? find_class_mahala.(eachrow(z_train)) : find_class_euclid.(eachrow(z_train))

# ╔═╡ 8dede4cf-7f18-4673-8af8-b778d63f8bc7
confusion_matrix(y_train, df_train.qualityC)

# ╔═╡ bdd51a4b-6578-43d0-bbbb-7b87acc2cba8
z_test = X_test * W

# ╔═╡ a78e05a5-1a22-4087-b653-d743311aac6c
y_test = use_mahalanobis ? find_class_mahala.(eachrow(z_test)) : find_class_euclid.(eachrow(z_test))

# ╔═╡ 59796912-1f02-4788-b972-ed578ea4ffee
confusion_matrix(y_test, df_test.qualityC)

# ╔═╡ 9827dd25-51f8-4921-be5d-357decef8f5a
accuracy(y_train, df_train.qualityC)

# ╔═╡ a769eb3f-e4d3-4e7f-a468-fa3e36390099
accuracy(y_test, df_test.qualityC)

# ╔═╡ 4933d90f-0234-4d76-8447-04672f4c0e69
begin
	color_map = Dict("low" => :red, "medium" => :yellow, "high" => :green)
	test_colors = map(x -> color_map[x], df_test.qualityC)
end

# ╔═╡ f0c37dbb-89a7-4c99-beb0-1626d1639910
begin
	plot = scatter()
	for quality in ["low", "medium", "high"]
		idx = findall(x -> x == quality, df_test.qualityC)
		scatter!(z_test[idx,1], z_test[idx,2], color=color_map[quality], markershape=:circle, label=quality)
	end
	# Plot the means of each class
	scatter!(z_low[1,:], z_low[2,:], 
         color=color_map["low"], 
         markershape=:star5,
		 markersize=12,
			 markerstrokewidth=2,
         label=L"\mu_{low}")
	scatter!(z_med[1,:], z_med[2,:], 
	...
	scatter!(z_high[1,:], z_high[2,:], 
	         color=color_map["high"], 
	         markershape=:star5, 
			 markersize=12,
			 markerstrokewidth=2,
			 label=L"\mu_{high}")
	plot!(xlabel="X", ylabel="Y", title="LDA Projected Values")
end

# ╔═╡ 541fd536-ce1c-4703-9e08-d397afa0a94a
size(z_test)
...
manifest_format = "2.0"
project_hash = "67e38a101fa2a8ced48827c7f1c5a8a1f15f13ff"
