...
# ╔═╡ c0a8510e-5d8d-11f0-0937-97cc10546826
using Plots, Statistics, MLJ, DataFrames, LinearAlgebra

# ╔═╡ 9e9a766a-c89b-4008-ab1b-0d20415ee40f
using PlutoUI, LaTeXStrings

# ╔═╡ 71ac69bc-f103-4931-b890-bb21435293fb
import CSV

# ╔═╡ fc77182b-53ba-4805-bd0c-3b8a6b83e856
@bind filtered_data CheckBox(default=false)

# ╔═╡ 89a96a22-fc39-48b3-bf22-5e9de7399483
begin
	...
	# Drop mediums for a 2 class problem
	df_train = filter(x -> x.qualityC != "medium", df_train)
	df_test = filter(x -> x.qualityC != "medium", df_test)
	size(df_train)
end

# ╔═╡ 7334bdaf-26b6-412f-a09d-8c964daf8286
features = names(df_train, Not([:quality, :qualityC]))

# ╔═╡ 8350c6ec-a5b3-4452-b57e-81c57b20ec6d
md"Let :high be 1, :low be 0"

# ╔═╡ a55d0bc5-d06c-4f4b-b46c-74b3038d8f1e
class_map = Dict("high" => 1.0, "low" => 0.0)

# ╔═╡ ab468dbd-c1da-4f56-810c-a807b360e78f
t_train = Matrix( map(x -> class_map[x], df_train.qualityC)' )'

# ╔═╡ 4fcaca47-5de1-4721-81e1-b908d9b8edf8
X_train = Matrix(df_train[:, features])

# ╔═╡ 3774f7cd-0cce-40a1-96b7-8065b4ec9846
X_test = Matrix(df_test[:, features])

# ╔═╡ 7defe96d-b8a5-45af-9748-2de7afc150d8
sigmoid(a) = @. 1 / (1 + exp(-a))

# ╔═╡ ae39c221-165b-4e64-87d3-f82610cfa2a4
function gradient_descent(X_train, w0, η, t, δ)
	w_old = w0
	w_new = w0
	...
	while first_iteration || norm(w_new - w_old) > δ # Test for convergence
		...
		a = X_train * w_old
		y = sigmoid(a)
		w_new = w_old - η * X_train' * (y - t)
		println(norm(y-t))
	end
	w_new
end

# ╔═╡ c6fc8cf4-fc68-431a-a93c-d06f7db7effe
begin
	w0 = rand(length(features))
	η = 0.001 # Learning rate
	δ = 0.0001 # convergence
	w_ml = gradient_descent(X_train, w0, η, t_train, δ)
end

# ╔═╡ 153ae2e2-d6e8-4ab4-8e4e-fce5c209134c
y_train = sigmoid(X_train * w_ml)

# ╔═╡ 8d756951-538b-4f0c-beee-c504cf190aa1
y_pred = ifelse.(y_train .> 0.5, "high", "low")

# ╔═╡ d899791d-3a25-46af-9c0f-ab62fc08c3d2
y_test = sigmoid(X_test * w_ml)

# ╔═╡ b679529e-a660-48fa-9923-0d4f8dc7120b
y_test_pred = ifelse.(y_test .> 0.5, "high", "low")

# ╔═╡ cf152d3e-6198-4218-86b1-31f4cfcec40c
accuracy(y_pred[:,1], df_train.qualityC)

# ╔═╡ 47081c91-dd61-431f-819e-b18576c5aa26
accuracy(y_test_pred[:,1], df_test.qualityC)
