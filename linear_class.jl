...
# ╔═╡ 945dde06-5a6e-11f0-22ee-255daa4ef16c
using CSV, DataFrames, Statistics, LinearAlgebra, Plots, MLJ

# ╔═╡ 7af885ef-fb2b-4f08-8926-f2722b12faa3
using PlutoUI

# ╔═╡ 905d7d51-02b9-47e6-9186-9172d4517ab2
@bind filter_data Select([true, false])

# ╔═╡ f51bec42-7b22-445f-80cf-767163c27772
begin
	df_train_tmp_all = CSV.read("train.csv", DataFrame)
	df_train_tmp_f = CSV.read("train_f.csv", DataFrame)
	df_train_tmp = filter_data ? df_train_tmp_f : df_train_tmp_all
	df_train = filter(x -> x.qualityC != "medium", df_train_tmp)
	df_test_tmp = CSV.read("./test.csv", DataFrame)
	df_test = filter(x -> x.qualityC != "medium", df_test_tmp)
end

# ╔═╡ 0d9d17c3-8663-4054-9666-2f74f1e7eb91
md"""
# Two class FDA

Let's just do the high and low class and see what kind of accuracy we get. So first I need my two mean vectors. Then I need my two sample covariances. Then I compute w, pick halfway between the means and check my training error.
"""

# ╔═╡ e411644b-d19c-4a90-92f6-836c43041a97
begin
	low_train = filter(x -> x.qualityC == "low", df_train)
	high_train = filter(x -> x.qualityC == "high", df_train)
end

# ╔═╡ 5c731ac9-7ed8-496f-9dae-24b6bfa095c3
features = names(df_train, Not([:quality, :qualityC]))

# ╔═╡ e8aeff74-60ce-46c9-b278-db1deb8d6e1e
begin
	low_matrix = Matrix(low_train[:, features])
	high_matrix = Matrix(high_train[:, features])
	μ_low = mean(low_matrix, dims=1)
	μ_high = mean(high_matrix, dims=1)
end

# ╔═╡ 84d5b04d-9cac-4f73-9239-ece5568843d6
begin
	low_centered = low_matrix .- μ_low
	high_centered = high_matrix .- μ_high

	σ_low = low_centered' * low_centered
	σ_high = high_centered' * high_centered

	S_w = σ_low + σ_high
end

# ╔═╡ b61c20e7-3fa8-41b0-8c16-4e6775454c4c
w = inv(S_w) * (μ_high' - μ_low')

# ╔═╡ c0c61351-ed8d-4064-86a1-26b58fbd2a75
w_norm = w ./ norm(w)

# ╔═╡ f4ef36f0-63ba-4bd7-ac0c-1e465520cf66
X_train = Matrix(df_train[:, features])

# ╔═╡ d0c0755b-0bb4-4b74-90e4-9c358260eeb7
X_class = Vector(df_train[:, :qualityC])

# ╔═╡ 21becdf1-7645-4261-9b6b-b0eaeebe3d94
m_low = μ_low * w

# ╔═╡ cccaa502-da92-43c3-bf7f-b74cb8b66696
m_high = μ_high * w

# ╔═╡ 24b43c91-fef6-4da8-b296-03a98a9ed869
δ = ((m_high + m_low) / 2)[1,1]

# ╔═╡ 09e11ed5-b1fc-4306-9c5a-c8e6258b2a22
X_train_pred = vec( String7.(ifelse.(X_train * w .> δ, "high", "low")))

# ╔═╡ eec6ff49-a2b7-416a-9df5-4c6154c76d7e
accuracy(X_train_pred, X_class)

# ╔═╡ 15b6939d-4d46-4f7d-af21-eacd8e1840ed
confusion_matrix(X_train_pred, X_class)

# ╔═╡ dc569c59-b697-4bd6-8925-2c4b09566955
X_train_proj = vec(X_train * w)

# ╔═╡ cdc8267f-f842-47c6-950c-6c9d0a95e3d0
p1 = scatter(X_train_proj, color=[x == "high" ? :red : :blue for x in X_class], markershape=:circle, label="y")

# ╔═╡ afcbec98-24cf-429e-acad-7e0a7bf39182
x = 1:length(X_train_proj)

# ╔═╡ 7299e246-e2f8-4a47-82b0-2f7a59eee26e
plot!(x, fill(δ, length(X_train_proj)), seriestype=:line, label="δ", linewidth=3, color=:green)

# ╔═╡ 1afad76c-061c-4402-8d3e-8a215f269b97
X_test = Matrix(df_test[:, features])

# ╔═╡ 4e057baa-3c33-44bf-83ec-b69c341bf606
X_test_pred = vec(String7.(ifelse.(X_test * w .> δ, "high", "low"))) # X_test_pred = X_test * w .> δ

# ╔═╡ 38eecfa1-1fc0-4050-bd08-ac21ae465152
X_test_class = Vector(df_test[:, :qualityC])

# ╔═╡ cc343e41-1be3-4c95-ab1e-853d7737cdd1
cm = confusion_matrix(X_test_pred, X_test_class)

# ╔═╡ 52fbe3b8-16f3-48cd-8c65-84c459d644a9
accuracy(X_test_pred, X_test_class)

# ╔═╡ 473a0e12-719d-4822-bef1-6a376212e680
X_test_proj = vec(X_test * w)

# ╔═╡ 16f03278-8eee-4c42-ace3-4e318e6b6e07
scatter(X_test_proj, color=[x == "high" ? :red : :blue for x in X_test_class], markershape=:circle, label="y")

# ╔═╡ 608ce579-127d-419d-be31-321e2bc6caa3
plot!(1:length(X_test_proj), fill(δ, length(X_test_proj)), seriestype=:line, label="δ", linewidth=3, color=:green)

# ╔═╡ 3bd4c220-73aa-4f67-a041-9abc53f2ad22
accuracy(X_train_pred, X_class)

# ╔═╡ 1b83977f-caad-4df0-81e9-fc1f2bf8fe5a
accuracy(X_test_pred, X_test_class)
...
manifest_format = "2.0"
project_hash = "9de9547f56bbc5dc21841bcfb928200c7c1db4c1"
