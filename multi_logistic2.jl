...
# ╔═╡ 5f63b95a-ee12-4057-82ae-b0b9a01f5fcf
using MLJ, Statistics, DataFrames, Plots, PlutoUI, LinearAlgebra

# ╔═╡ 54e15542-590e-4c8c-85b1-ec9106ae6c5e
using BlockArrays

# ╔═╡ 0dc6b7b5-9c6c-4412-970f-e514e3a3ab36
using BenchmarkTools

# ╔═╡ 6481c036-5e0a-11f0-250d-57ecba01301d
import CSV

# ╔═╡ fd9652ce-de7a-469a-8e11-635374d6eb2f
@bind drop_outliers CheckBox(default=false)

# ╔═╡ 1e40941e-09a4-4c26-ba84-9d4cfe8b73ee
begin
	df_train_all = CSV.read("train.csv", DataFrame)
	df_train_drop_outliers = CSV.read("train_f.csv", DataFrame)
	df_train = drop_outliers ? df_train_drop_outliers : df_train_all
	df_test = CSV.read("test.csv", DataFrame)
end

# ╔═╡ 47b79c2f-7f1e-491a-a3b2-2478b4e30722
features = names(df_train, Not([:quality, :qualityC]))

# ╔═╡ 397c5245-eade-45dc-baa3-22f0993e8a98
X_train = Matrix(df_train[:, features])

# ╔═╡ 52def8dc-9a6e-4c3b-ae93-f832c526c4f7
X_test = Matrix(df_test[:, features])

# ╔═╡ a606ff46-ab21-4a19-bdff-ea2d86ff24f3
N = nrow(df_train)

# ╔═╡ dbedc883-9393-4bfe-a66b-2d7236383cab
function one_hot_encode(df)
	classes = ["low", "medium", "high"]
	T = zeros(Float64, nrow(df), length(classes))
	for (i, row) in enumerate(eachrow(df))
	    class_index = findfirst(==(row.qualityC), classes)
	    T[i, class_index] = 1
	end
	T
end

# ╔═╡ 3d8a52f6-78ba-4a59-a6a1-7a2ddc2c4b56
T = one_hot_encode(df_train)

# ╔═╡ 384e6ff0-d8bb-4e09-81da-db7ad67ded74
M = length(features)

# ╔═╡ 5d15bf7e-66ff-42fe-861a-dc5ee35fd7af
function predict_logreg(w_vec, X)
	W = reshape(w_vec, M, 3)
	a = X * W
	y_mat = exp.(a) ./ sum(exp.(a), dims=2) # Softmax
	return y_mat
end

# ╔═╡ a0612da5-6609-4cf7-9b62-6c6602bc1580
function compute_g(w_vec)
	y_mat = predict_logreg(w_vec, X_train)
	g_mat = X_train' * (y_mat - T)
	vec(g_mat)
end

# ╔═╡ 4a5e262a-c744-4d8a-abb8-fda19f24548f
function gradient_descent(w0, η, δ)
	w_old = w0
	w_new = w0
	first_iteration = true
	iter = 0

	# I prefer using a first_iteration flag instead of a while true formulation
	while first_iteration || norm(w_new - w_old) > δ # Test for convergence
		first_iteration = false
		w_old = w_new
		w_new = w_old - η * compute_g(w_old)
		#println(norm(w_new - w_old))
		iter += 1
	end
	println(iter)
	w_new
...
# ╔═╡ bcc824ca-811f-4af9-b306-747035c4e7c2
begin
	w0 = zeros(3*M)
	η = 0.0001 # Learning rate
	δ = 0.00001 # convergence
	w_ml = gradient_descent(w0, η, δ)
end

# ╔═╡ 0b4bbc4f-b9df-43fe-8143-c6f3e6b433a5
function compute_H(w_vec)
	W = reshape(w_vec, M, 3)
	a = X_train * W
	y_mat = exp.(a) ./ sum(exp.(a), dims=2) # Softmax

	#blocks = Matrix{Matrix{Float64}}(undef, 3, 3) # under doesn't initialize the matrix to any values

	blocks = Matrix{Float64}(undef, 3*M, 3*M)
	for j in 1:3
		for k in j:3
			I_jk = j == k ? ones(Float64, N, 1) : zeros(Float64, N, 1)
			R = diagm(vec(y_mat[:, j] .* (I_jk - y_mat[:, k])))
			block = X_train' * R * X_train
			#blocks[j,k] = block
			#if j != k
			#	blocks[k,j] = blocks[j,k]'
			#end

			blocks[(j-1)*M+1:j*M, (k-1)*M+1: k*M] = block
			if j != k
				blocks[(k-1)*M+1:k*M, (j-1)*M+1: j*M] = block'
			end
		end
	end

	# Construct H using mortar
	#H = Matrix(mortar(blocks))
	
	#return H
	return blocks
end

# ╔═╡ 4d0b55e1-641e-40ee-b1d3-34a3ed289424
w_vec_tmp = zeros(3*M)

# ╔═╡ c33fd516-deab-4aa2-b6c6-c01389f32bff
# ╠═╡ disabled = true
#=╠═╡
@benchmark compute_H($w_vec_tmp)
  ╠═╡ =#

# ╔═╡ 4fcb17fb-591c-4dad-ac35-869f95ce815e
# ╠═╡ disabled = true
#=╠═╡
@benchmark compute_g($w_vec_tmp)
  ╠═╡ =#

# ╔═╡ 4e9cc280-5a88-4cdd-b6d4-fde03f9bf3bb
function newton_raphson(w0, δ, λ)
	w_old = w0
	w_new = w0
	first_iteration = true
	iter = 0
	...
	while first_iteration || norm(w_new - w_old) > δ # Test for convergence
		...
		H = compute_H(w_old)
		g = compute_g(w_old)
		identity = Matrix(I, 3*M, 3*M)
		
		Δw = - inv(H + λ*identity) * g
...
# ╔═╡ 383e2c52-ecfa-409e-af6d-909d5ccee54d
begin # Newton-Raphson method
	λ = 0.1 # Diagonal ridge for numerical stability when inverting the hessian
	w_ml_nr = newton_raphson(w0, δ, λ)
end

# ╔═╡ 307974e5-197d-489f-ab0b-046405a6a5b3
[w_ml' ; w_ml_nr']

# ╔═╡ d8b5c633-fafa-46f5-8bd6-21d4c1904df8
isapprox(w_ml, w_ml_nr; atol=1e-2) # Checks if the solutions converged on the same number

# ╔═╡ cbd13a81-dde8-4734-8800-62119907e466
y_train = predict_logreg(w_ml, X_train)

# ╔═╡ 91c8ec00-b2ff-46ef-b78c-ba3eec15ab15
y_test = predict_logreg(w_ml, X_test)

# ╔═╡ 3d72e5e2-6f67-435d-bb93-5a12f443bf29
index_map = Dict( 1 => "low", 2 => "medium", 3 => "high")

# ╔═╡ 49e9a526-a47c-4cd2-b942-6ab2d1e4a956
y_train_pred = map(i -> index_map[i], argmax.(eachrow(y_train)))

# ╔═╡ 07bd004d-55ad-4ff5-9425-a4c841bc5a2a
y_test_pred = map(i -> index_map[i], argmax.(eachrow(y_test)))

# ╔═╡ 642f121d-231f-4986-b1be-4b1f72542f5b
accuracy(y_train_pred, df_train.qualityC)

# ╔═╡ f061a773-e210-4a0e-944c-9c412347c47f
accuracy(y_test_pred, df_test.qualityC)
...
manifest_format = "2.0"
project_hash = "76f0f2a2ec8d59cc8ddb7b36cdb8642f84496ad6"
